...
const QString rfa_url = "http://10.4.23.76/cgi-bin/post_manager";
...
const QString request_data = "<Command><Name>device_list</Name></Command>\n";
...
RFADeviceList::RFADeviceList(QWidget *parent): QDialog(parent), reply(0) {
	...
	connect(&qnam, &QNetworkAccessManager::authenticationRequired, this, &RFADeviceList::slotAuthRequired);
...
void RFADeviceList::get(const QUrl &url) {
	QNetworkRequest request(url);
	QByteArray reqData(request_data);
...
void RFADeviceList::metaDataChanged() {
	QUrl redirTarget;
...
void RFADeviceList::finished(QNetworkReply *reply) {
	Q_UNUSED(reply);
...
QString RFADeviceList::itemActivated(QTreeWidgetItem *item) {
	return (item->text(0));
}

void RFADeviceList::error(QNetworkReply::NetworkError) {
...
void RFADeviceList::slotAuthRequired(QNetworkReply*, QAuthenticator *authenticator) {
	QDialog authDialog;
